		...
		<div class="container">
			<div class="row">
				<div class="col-md-8 m-auto">
					...
					<p class="lead text-center">请填写你的个人资料，让更多人去了解</p>
					...
					<form @submit.prevent="submit">
						<TextField
							...
							:error="errors.handle" 
							info="此处的handle是在后端接口中用来查询的数据,通常是写你的email名字">
						</TextField>
						<SelectList
							name="status" 
							:error="errors.status" 
						...
						<div class="mb-3">
							<button type="button" class="btn btn-light"
								@click="dispalaySocialInputs = !dispalaySocialInputs"
							>添加社交账号</button>
							<span class="text-muted">选项</span>
						</div>
						<div v-show="dispalaySocialInputs">
							<InputGroup
								placeholder="微信公众号"
								name="wechat"
...
export default {
  ...
  data () {
	...
	methods:{
		submit() {
			this.$axios.post('/api/profile',this.msgInfo).then(res => {
				this.errors = {}
				//存储数据
		...
		dispalaySocialInput() {
			this.dispalaySocialInputs = !this.dispalaySocialInputs
		}
	},
  components:{
		TextField,
